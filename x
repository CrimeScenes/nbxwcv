...
local function getHitboxOffset(distance)
    ...
    -- Ensure the hitbox size works for all distances, including close range
    if detectionZone.x == 3.8 and detectionZone.y == 2.6 and detectionZone.z == 3.6 then
        if distance <= 5 then
            hitboxSize = Vector3.new(0, 1.5, 0)  -- For very close range targets, use a larger offset
        elseif distance <= 25 then
            hitboxSize = Vector3.new(0, 1, 0)  -- Close range hitbox size
        elseif distance <= 41 then
            hitboxSize = Vector3.new(0, 0.8, 0)  -- Mid range hitbox size
        else
            hitboxSize = Vector3.new(0, 1.4, 0)  -- Far range hitbox size
        end
    end
    return hitboxSize
end
...
local function predictTargetPosition(targetPlayer, deltaTime)
    ...
    if humanoidRootPart then
        ...
        if distance <= 5 then
            predictionFactor = 0.05  -- For very close range
        elseif distance <= 25 then
            predictionFactor = 0.1  -- Close range
        elseif distance <= 41 then
            predictionFactor = 0.2  -- Mid range
        else
            predictionFactor = 0.3  -- Far range
        end
...
local function airSmoothing(predictedPos, targetPlayer)
    ...
    if humanoid then
        if humanoid:GetState() == Enum.HumanoidStateType.Jumping or humanoid:GetState() == Enum.HumanoidStateType.Physics then
            -- If in air, smooth the Y position based on velocity
...
-- Function to aim at legitimate hitpoints
local function aimAtTargetHitbox(targetPlayer)
    ...
    -- Get the screen position of the predicted hitbox
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if humanoid and rootPart then
            local ray = Ray.new(rootPart.Position, Vector3.new(0, -1, 0) * 4) 
            local hit, position = workspace:FindPartOnRay(ray, player.Character)

            if hit then
               
                adjustedVelocity = adjustedVelocity * 0.8 
            else
           
                local gravityFactor = 1 - math.abs(adjustedVelocity.Y) * 0.01
                adjustedVelocity = adjustedVelocity + Vector3.new(0, -workspace.Gravity * 0.05, 0) * gravityFactor
            end
        end
...
RunService.RenderStepped:Connect(function()
    ...
    if shared.Forbidden['Silent Precision'].IsActive and IsTargeting then  
        ...
        if TargetPlayer then
            if TargetPlayer.Character then
                local targetPos = TargetPlayer.Character:FindFirstChild("Head") and TargetPlayer.Character.Head.Position
                if not targetPos or TargetPlayer.Character.Humanoid.Health < 1 then
                    TargetPlayer = nil
                    IsTargeting = false
                    LastTarget = nil 
                    return
                ...
                if closestPart and closestPoint then
                    ...
                    -- Ground/Air Prediction Logic
                    local rootPart = TargetPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if rootPart then
                        local ray = Ray.new(rootPart.Position, Vector3.new(0, -1, 0) * 4)
                        local hit, _ = workspace:FindPartOnRay(ray, TargetPlayer.Character)

                        if hit then
                            -- Grounded: Apply ground friction
                            velocity = velocity * 0.8
                        else
                            -- Airborne: Simulate gravity adjustment
                            local gravityFactor = 1 - math.abs(velocity.Y) * 0.01
                            velocity = velocity + Vector3.new(0, -workspace.Gravity * 0.05, 0) * gravityFactor
                        end
                    end

                    -- Time distortion adjustments
                    ...
                    -- Fire the remote event with adjusted velocity
    ...
    elseif LastTarget and LastTarget.Character then
        local lastTargetPos = LastTarget.Character:FindFirstChild("Head") and LastTarget.Character.Head.Position
        if lastTargetPos and IsVisible(lastTargetPos) then
